##### sumo_env for double intersection network with 1 agent
import os
import sys
...
import numpy as np
import pandas as pd
from tl_agent import Traffic_Light
...
class SumoEnvironments(gym.Env):
    ...
    def step(self, action):
        ...
        if action is None or action == {}:
            ...
            act = False
            time_to_act = {'1': False, 'gneJ13': False}
            while not act:
                self.sumo_step()
                ...
                for ts in self.ts_ids:
                    ...
                    if self.traffic_lights[ts].time_to_act:
                        time_to_act.update({ts : True})
                        if time_to_act['1'] and time_to_act['gneJ13']:
                            act = True
        ...
        reward = ( rewards['1'] + rewards['gneJ13'])
    ...
    def compute_observations(self):
        x = dict
        x = {ts: self.traffic_lights[ts].compute_observation() for ts in self.ts_ids }
    ...
    def compute_rewards(self):
        return {ts: self.traffic_lights[ts].compute_reward() for ts in self.ts_ids }
    
    def observation_spaces(self, ts_id):
        return   self.traffic_lights[ts_id].observation_space
    
    def action_spaces(self, ts_ids):
        return self.traffic_lights[ts_id].action_space
    
    @property
    def obs_space(self):
        return spaces.Box(low=np.zeros(self.traffic_lights['1'].num_green_phases + 2*len(self.traffic_lights['1'].lanes), dtype=np.float32), high=np.ones(self.traffic_lights['1'].num_green_phases + 2*len(self.traffic_lights['1'].lanes)), dtype=np.float32)

    
    @property
    def act_space(self):
        return spaces.Discrete(self.traffic_lights['1'].num_green_phases)
    ...
    def encode(self, state):
        phase1 = [state[0]]
        phase2 = [state[5]]
        density_queue1 = [self._discretize_density(d) for d in state[1:5]]
        density_queue2 = [self._discretize_density(d) for d in state[6:11]]

        return tuple(phase1 + phase2 + density_queue1 + density_queue2)
